...
int dump_file(int img, int inode_nr, int out) {
    ...
    if (read(img, sb, sizeof(struct fs_superblock)) != sizeof(struct fs_superblock)) {
        fs_xfree(sb);
        return -EIO;
    }
    ...
    uint32_t inode_table_offset = 2048 + (block_group * sb->s_inode_size * sb->s_inodes_per_group);
    uint32_t inode_offset = inode_table_offset + (inode_index * sb->s_inode_size);
    ...
    // Get total file size (combining upper and lower 32 bits)
    uint64_t total_size = inode->size_lower | ((uint64_t)inode->size_upper_or_directory_acl << 32);
